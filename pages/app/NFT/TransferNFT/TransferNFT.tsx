...
import TransferNFTBlock from 'components/blocks/NFT/TransferNFTBlock/TransferNFTBlock'
...
const TransferNFT: NextPage = () => {
    ...
    try {
      ...
      try {
        if (res.isInBlock && !res.isFinalized) {
          ...
          const event = res.events.find((x) => x.event.method === 'NFTTransferred')
          const nftId = event && Number.parseInt(event.event.data[0].toString())
          const isSuccess = isTransactionSuccess(res).success
          if (isSuccess && nftId) {
            dispatch(actions.setUserNFTs(NFTs.filter(({ value }) => value !== nftId)))
          }
          setResponse({
  ...
  return (
    <>
      <TransferNFTBlock signableCallback={signableCallback} />
      {unsignedTx && (
