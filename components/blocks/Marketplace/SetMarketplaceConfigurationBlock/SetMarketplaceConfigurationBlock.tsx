...
type IForm = {
  id: number
  commissionFee: CommissionFeeType
  listingFee: ListingFeeType
  accountList: AccountListType
  offchainData: OffchainDataType
}
...
const SetNFTMarketplaceConfigurationBlock = ({ signableCallback }: Props) => {
  ...
  const onSubmit: SubmitHandler<IForm> = async ({ id, commissionFee, listingFee, accountList, offchainData }) => {
    const setMarketplaceConfigurationTxHex = await setMarketplaceConfigurationTx(id, commissionFee, listingFee, accountList, offchainData)
